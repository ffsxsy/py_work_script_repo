    ...
    def sync_running(self, devices: list[DeviceSession]) -> list[str]:
        ...
        for key in list(self._runtimes):
            if key not in wanted_groups:
                self._runtimes.pop(key).stop()
                continue
            # Restart if unit set or params changed
            ...
            if old_ids != new_ids or runtime.link.to_dict() != link.to_dict():
                self._runtimes.pop(key).stop()

        # Start missing
        for key, group in wanted_groups.items():
            if key in self._runtimes:
                # refresh device refs / values into context
                rt = self._runtimes[key]
                for d in group:
                    rt.devices[d.id] = d
                    try:
                        ctx = rt.context[int(d.unit_id)]
                    except Exception:  # noqa: BLE001
                        ctx = None
