...
class ZCAN_CAN_FRAME(Structure):
    # 与 can_OTA_1218 / Linux can_frame 位域一致：独立 eff/rtr（勿只用 can_id bit31）
    _fields_ = [
        ("can_id", c_uint, 29),
        ("err", c_uint, 1),
        ("rtr", c_uint, 1),
        ...
        (
            "_pad",
            c_ubyte,
        ),  # bit5 发送回显 bit6 队列发送时间精度 bit7 队列发送
        ("_res0", c_ubyte),
        ("_res1", c_ubyte),
        ("data", c_ubyte * 8),
    ]
...
class ZCAN_CANFD_FRAME(Structure):
    ...
    _fields_ = [
        ...
        (
            "flags",
            c_ubyte,
        ),  # 0-开启 1-关闭; bit5 设置发送回显 bit6 设置队列发送时间精度(是否设置0.1ms精度，默认精度是1ms)
...
class ZCAN_DYNAMIC_CONFIG_DATA(Structure):
    _pack_ = 1
    _fields_ = [("key", c_char * 64), ("value", c_char * 64)]
...
class ZCANFD_AUTO_TRANSMIT_OBJ_PARAM(Structure):  # auto_send delay
    _fields_ = [("indix", c_ushort), ("type", c_ushort), ("value", c_uint)]
...
class ZCAN(object):
    ...
    def ReceiveData(self, device_handle, rcv_num, wait_time=c_int(-1)):
        try:
            ...
            ret = self.__dll.ZCAN_ReceiveData(
                device_handle, byref(rcv_can_data_msgs), rcv_num, wait_time
            )
