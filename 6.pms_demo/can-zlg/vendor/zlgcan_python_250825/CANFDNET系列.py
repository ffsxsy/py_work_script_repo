...
def start_server_only(can_number=1):
    ...
    ret = zcanlib.ZCAN_SetValue(
        device_handle, "0/work_mode", "1".encode("utf-8")
    )  # 0-客户端 1-服务器
    ...
    for i in range(can_number):
        ret = zcanlib.ZCAN_SetValue(
            device_handle, str(i) + "/set_device_tx_echo", "1".encode("utf-8")
        )  # 发送回显设置，0-禁用，1-开启
        ...
        if ret == ZCAN_STATUS_ERR:
            print(
                f"StartCAN{i} fail ,连接设备失败，请确保目标IP和目标端口是否填写正确，并确保PC与以太网CAN卡处于同一网段"
            )
...
def start_server_all(can_number=1):
    ...
    for i in range(can_number):
        ...
        ret = zcanlib.ZCAN_SetValue(
            device_handle, str(0) + "/set_device_tx_echo", "1".encode("utf-8")
        )  # 发送回显设置，0-禁用，1-开启
        if ret != ZCAN_STATUS_OK:
...
def start_client_only(can_number=1):
    ...
    # 没写则不需要设置
    ret = zcanlib.ZCAN_SetValue(
        device_handle, "0/work_mode", "0".encode("utf-8")
    )  # 0-客户端 1-服务器
    if ret == ZCAN_STATUS_ERR:
        print("设置工作模式失败！")

    ret = zcanlib.ZCAN_SetValue(
        device_handle, "0/work_port", work_port[0].encode("utf-8")
    )  # 设置目标端口
    if ret == ZCAN_STATUS_ERR:
        print("设置目标端口失败！")
...
def start_client_all(can_number=1):
    ...
    for i in range(can_number):
        ...
        ret = zcanlib.ZCAN_SetValue(
            device_handle, "0/work_port", work_port[i].encode("utf-8")
        )  # 目标端口
        if ret == ZCAN_STATUS_ERR:
            print("设置目标端口失败！")
...
def start_UDP_only(can_number=1):
    ...
    # 没写则不需要设置
    ret = zcanlib.ZCAN_SetValue(
        device_handle, "0/local_port", local_port[0].encode("utf-8")
    )  # 设置本地端口
    if ret == ZCAN_STATUS_ERR:
        print("设置本地端口失败！")
...
def start_UDP_all(can_number=1):
    ...
    for i in range(can_number):
        ...
        # 没写则不需要设置
        ret = zcanlib.ZCAN_SetValue(
            device_handle, "0/local_port", local_port[i].encode("utf-8")
        )  # 设置本地端口
        if ret == ZCAN_STATUS_ERR:
            print("设置本地端口失败！")
    ...
    while thread_flag:
        ...
        if rcv_canfd_num:
            ...
            with print_lock:
                for msg in rcv_canfd_msgs[:rcv_canfd_num]:
                    ...
                    frame_type = "扩展帧" if frame.can_id & (1 << 31) else "标准帧"
                    frame_format = (
                        "远程帧" if frame.can_id & (1 << 30) else "数据帧"
                    )  # CANFD没有远程帧
                    can_id = hex(frame.can_id & 0x1FFFFFFF)
        ...
        if rcv_merge_num:
            if rcv_num > 100:
                ...
                rcv_merger_msgs, rcv_merge_num = zcanlib.ReceiveData(
                    device_handle, rcv_merge_num, 100
                )
            with print_lock:
                for msg in rcv_merger_msgs[:rcv_merge_num]:
                    if msg.dataType == ZCAN_DT_ZCAN_CAN_CANFD_DATA:
                        ...
                        print(
                            f"[{msg.zcanfddata.timestamp}] CAN{msg.chnl} {can_type:<{CANType_width}}\t{direction} ID: {hex(can_id):<{id_width}}\t{frame_type} {frame_format}"
                            f" DLC: {frame.len}\tDATA(hex): {data}"
                        )
    print("=====")
...
def Dynamic_config_test(device_handle, chn):
    ...
    CfgData.key = ZCAN_DYNAMIC_CONFIG_CAN_NOMINALBAUD(chn).encode(
        "utf-8"
    )  # b"DYNAMIC_CONFIG_CAN0_NOMINALBAUD"#
...
def Queue_Transmit_Test(device_handle, chn, chn_handle):
    ...
    for i in range(transmit_num):
        ...
        msgs[i].frame.can_id |= (
            1 << 31
        )  # 最高位(bit31)为 扩展帧/标准帧 标识位 同理 bit30为 数据帧/远程帧
        msgs[i].frame.can_dlc = 8  # 数据长度
        msgs[i].frame._pad |= 0x20  # 发送回显
        msgs[i].frame._pad |= 1 << 7  # 设置队列发送
        msgs[
            i
        ].frame._res0 = 10  # 10ms res0，res1共同表示队列发送间隔(可以理解为一个short 2Byte分开传入)
...
def Clear_Send_Task(device_handle, chn):
    ...
    ret = zcanlib.ZCAN_SetValue(
        device_handle, str(chn) + "/clear_delay_send_queue", "0".encode("utf-8")
    )
    if ret != ZCAN_STATUS_OK:
...
def Transmit_Test(chn_handle):
    ...
    for i in range(transmit_num):
        ...
        msgs[
            i
        ].frame._res0 = 10  # res0，res1共同表示队列发送间隔(可以理解为一个short 2Byte分开传入)
        msgs[i].frame._res1 = 0
        for j in range(msgs[i].frame.can_dlc):
            msgs[i].frame.data[j] = j
    ret = zcanlib.Transmit(chn_handle, msgs, transmit_num)
    with print_lock:
        print("成功发送 %d 条CAN报文" % ret)
...
if __name__ == "__main__":
    ...
    for i in range(len(handles)):
        # 设置使能合并接收
        ret = zcanlib.ZCAN_SetValue(
            handles[i], str(0) + "/set_device_recv_merge", repr(enable_merge_receive)
        )
        if ret != ZCAN_STATUS_OK:
    ...
    if len(handles) == 1:  # 当只打开一次设备 连接所有通道时，可以通过合并接收收取所有通道的数据
        if enable_merge_receive == 1:  # 若开启合并接收，所有通道都由一个接收线程处理
            thread = threading.Thread(
                target=receive_thread, args=(handles[0], chn_handles[0])
            )  # 开启独立接收线程
            ...
            for j in range(len(chn_handles)):
                thread = threading.Thread(
                    target=receive_thread, args=(handles[0], chn_handles[j])
                )  # 开启独立接收线程
                threads.append(thread)
                thread.start()
    else:
        ...
        for i in range(len(handles)):
            thread = threading.Thread(
                target=receive_thread, args=(handles[i], chn_handles[i])
            )  # 开启独立接收线程
            threads.append(thread)
            thread.start()
