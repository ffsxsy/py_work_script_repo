...
def Clear_Auto_Send(device_handle, chn):
    if is_auto_send:
        ret = zcanlib.ZCAN_SetValue(
            device_handle, str(chn) + "/clear_auto_send", "0".encode("utf-8")
        )
        if ret != ZCAN_STATUS_OK:
...
def Redirect_Test(device_handle, input_chn, out_chn):
    ...
    ret = zcanlib.ZCAN_SetValue(
        device_handle, str(input_chn) + "/redirect", (str(out_chn) + " 1").encode("utf-8")
    )
    if ret != ZCAN_STATUS_OK:
...
if __name__ == "__main__":
    ...
    # 打开设备
    handle = zcanlib.OpenDevice(ZCAN_USBCAN_4E_U, 0, 0)
