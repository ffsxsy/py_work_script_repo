...
local function is_fault_wire_id(wire_id)
    if type(bms20_is_fault_wire_id) == "function" then
        return bms20_is_fault_wire_id(wire_id)
    end
    return wire_id == 0x0329 or wire_id == 0x0213 or wire_id == 0x0109
end
...
local function dissect_one_frame(tvb, pinfo, tree, frame_index, service_port)
    ...
    local transport_type = tvb(9, 1):uint()
    ...
    if payload_len > 0 then
        ...
        local fault_profile_key = nil
        if type(bms20_fault_route_key) == "function" then
            fault_profile_key = bms20_fault_route_key(service_port, cmd_group, cmd_id)
        end
        if type(bms20_resolve_payload_msg_name) == "function" then
            payload_msg = bms20_resolve_payload_msg_name(display_name or msg_name, wire_id)
        end
        ...
        if not parsed and is_fault_wire_id(wire_id) and bms20_fault_profiles == nil and frame_tree ~= nil then
            frame_tree:add_expert_info(PI_PROTOCOL, PI_NOTE,
        ...
        if not parsed and fault_profile_key == nil and not is_fault_wire_id(wire_id)
            ...
            parsed = bms20_dissect_payload(
                payload_msg, payload_tvb, app_tree, frame_tree, pinfo, service_port)
        end
        if not parsed then
