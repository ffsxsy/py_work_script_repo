...
**使用者只需：** 安装 Wireshark → 复制 **10 个** `.lua` 文件 → Reload → 打开抓包过滤 `tcp.port >= 5001 && tcp.port <= 5014`。
...
├── bms20_fault_enabled.lua             ← 故障解析白名单（必装）
├── bms20_fault_defs.lua                ← 故障位图定义（必装，自动生成）
├── bms20_fault.lua                     ← 故障解析运行时（必装）
├── bms20_payload_enabled.lua           ← Payload 白名单（必装）
...
plugins/
├── bms20_msg_map.lua
├── bms20_fault_enabled.lua
├── bms20_fault_defs.lua
├── bms20_fault.lua
...
来源：各 `SystemConfiguration_*.xlsm` 的 **FaultList** 工作表（FaultID = 位图 bit 索引）。白名单见 `bms20_fault_enabled.lua`。
...
载荷均为 **25 B / 200 bit**。详情树显示 **Fault Bitmap** 与 **Active Faults**（仅列出置位故障，含 FaultName 与描述）。
...
| 0x03 | 0x02 | RBMS_Volt |
| 0x04 | 0x01 | RBMS_Fault（5003..5014）或 BBMS_Fault (M核)（5002） |
...
| `gen_fault_defs.py` | `bms20_fault_defs.lua`（RBMS / BBMS M核 / BBMS A核 故障表） |
...
- **Payload 深度解析** 当前白名单 4 条；故障位图另由 `bms20_fault_enabled.lua` 控制 3 条 profile
- `(0x04, 0x01)` 在矩阵中合并为 `BBMS_Fault / RBMS_Fault`；插件按端口分别显示 **RBMS_Fault** 或 **BBMS_Fault (M核)**
